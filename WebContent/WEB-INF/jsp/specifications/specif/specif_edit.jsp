...
<div class="main-container" id="main-container">
	...
	<input type="hidden" id="len"  name="len"  value="${fn:length(list)}"/>
	<input type="hidden" id="TPSG_ID" name="TPSG_ID" value="${pd.TPSG_ID}"/>
		<div class="options">
			<table class="table">
			    <tbody>
				    <tr>
				        ...
				        <td>
				        	<select id="TPSG_PRODUCT_CATEGORY_ID" value="" name="TPSG_PRODUCT_CATEGORY_ID">
				        		...
				        		<c:forEach items="${list2}" var="level">
									 <option value="${level.TPC_ID }" <c:if test="${level.TPC_ID == pd.TPSG_PRODUCT_CATEGORY_ID}">selected</c:if>>${level.TPC_NAME }</option>
									</c:forEach>
				    ...
				    <tr>
				        ...
				        <td>
				        	<input id="TPSG_NAME" name="TPSG_NAME" type="text" placeholder="请输入名称" value="${pd.TPSG_NAME}" />
				        </td>
				    </tr>
				    <tr>
				        ...
				        <td>
				        	<select id="TPSG_TYPE" name="TPSG_TYPE">
				        		<option value="请选择规格组名称...">请选择规格组名称...</option>
				        		<c:if test="${pd.TPSG_TYPE ==1}">
				    ...
				    <tr>
				        ...
				        <td>
				        	<input id="TPSG_SORT" name="TPSG_SORT" type="text" placeholder="请输入序列号" value="${pd.TPSG_SORT}" />
				        </td>
				    </tr>
				    <tr>
				        <td width="27%"></td>
...
<form  action="" name="Form" id="Form" method="post" id="parentFormId">
				...
				<table class="table" id="tbl">
					...
					<tbody>
				     <c:forEach items="${list}" var="level1" varStatus="stat">
						<tr>
						    <td style="display:none" width="1%"><input id ="a" type="hidden" value="${level1.TPSV_ID }" name="a"  /></td>
		...
		<button onclick="text()" class="allSubmit">提交</button>
</form>
...
$(".addTableName").click(function(e) {
	...
	if(classification==''||classification=='请选择商品分类...'){
		...
		alert("请输入名称！");
		return;
	}else if(specification==''||specification=='请选择规格组名称...'){
		alert("请选择规格组名称！");
		return;
		...
		var html='<tr>'+
					'<td width="20%"><input type="text" value="'+typeName+'" name="addName" class="addName" /></td>'+
					'<td width="5%" class="nothing"></td>'+
					'<td width="35%" class="addUploadBox"><img src="static/images/add.jpg" /><input type="file" name="addUpload" /></td>'+
					'<td width="5%" class="nothing"></td>'+
					'<td width="20%"><input type="text" name="addSorting" class="addSorting" value="'+sorting+'" /></td>'+
					'<td width="5%" class="nothing"></td>'+
					'<td width="10%"><input type="button" value="删除" class="addDelete" /></td>'+
				'</tr>';
...
function text(){
	...
	//var d='';
    var i=0;
	$("#tbl input[name='a']").each(function(){
		i=i+1
		if(i==1){
			a=$(this).val();
		}if(i==2){
